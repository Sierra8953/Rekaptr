...
### Core logic (`src/core/`)
"What the app does," separated from the UI that drives it (mirrors how `cloud/` is layered). Modules here are plain functions over plain inputs that report progress/results through `AppState`, so the UI stays a thin layer that gathers state, calls in, and renders:
- `core/export.rs` — `run_export(ExportParams, AppState)`: the ffmpeg command building, CUDA→software fallback, progress reader thread, thumbnail extraction, and concat-list cleanup. `ui/export.rs::perform_export` freezes inputs into `ExportParams` and calls it from a background task; the UI keeps mark validation, toasts, output-path derivation, and result/state handling.
- `core/recording.rs` — the `AppState`-only halves of the record feature: `clear_mic_subscribers`, `emergency_stop` (the `toggle_recording_internal` teardown), `begin_graceful_teardown` (the `stop_recording` teardown thread), and `notify_recording_state` (the tray + overlay sync every stop/start path must perform). The pipeline *launch* stays in `ui/recording.rs::start_recording` because it's entangled with per-step toasts and the bus monitor (needs `cx`).

### Capture & encode (`src/engine.rs`, `src/ui/recording.rs`, `src/core/recording.rs`)
The pipeline is built as a GStreamer launch string: `d3d11screencapturesrc` → NVENC encoder (H.264/HEVC/AV1) → `splitmuxsink` with `isofmp4mux`, writing fragmented `seg_%010d_*.m4s` segments (~6s each) into a per-title directory under the storage root. `engine.rs` also boosts process GPU scheduling priority (`D3DKMTSetProcessSchedulingPriorityClass`) so encoding doesn't starve the game. Pipeline bus messages are watched in `recording.rs::spawn_bus_monitor` — EOS ends a clean stop; a bus **Error** triggers the emergency-stop path (`toggle_recording_internal` → `core::recording::emergency_stop`).
...
### Cross-session timestamps (`src/utils/playlist.rs`)
...
`utils` is a directory of focused submodules behind a re-export façade (`utils/mod.rs`), so callers keep using stable `crate::utils::*` paths: `paths` (storage root, `clean_title`, bundled-binary lookup), `startup` (Run-key toggle), `segments` (fMP4 filename/ffprobe primitives), `playlist` (HLS + cross-session offset + clip marks/concat), `steam_assets` (appid → icon/cover/artwork), `clips_index` (source discovery, saved-clip library, `source_stats`), `retention` (rolling-buffer cleanup + global GB cap).

### UI (`src/ui/`)
Single root entity `RekaptrWorkspace` with an `active_view` switch (dashboard, clips, settings, etc.). `ui/mod.rs` is the workspace shell (struct, initializer, `Render` impl, and the cross-cutting methods); shared free helpers/enums (`ActiveView`, `SettingsTab`, `toggle_switch`, `track_color`, `prettify_process_name`, …) live in `ui/shared.rs`, re-exported via `pub use shared::*` so call sites keep the `crate::ui::*` paths. Views are modules under `src/ui/` (`clips.rs`, `export.rs`, `recording.rs`, `add_source.rs`, `setup_wizard.rs`), `src/ui/settings/`, and two view directories split by concern: `ui/dashboard/` (`mod` shell + `preview`/`mixer`/`sources` panes) and `ui/teams/` (`mod` types + `data` fetch/actions + `view` render). GPUI is immediate-mode-ish: render functions rebuild the element tree each frame, so per-frame work in render paths is hot (e.g. `Clip::path_str` is precomputed to avoid re-allocating every frame).
