...
impl LumaWorkspace {
    pub fn render_dashboard(&self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let rec_elapsed = self.recording_start_time
            ...
            .unwrap_or(0);
        let rec_bitrate = *self.app_state.rec_stats.bitrate_kbps.lock();
        let rec_dropped = self.app_state.rec_stats.dropped_frames.load(std::sync::atomic::Ordering::Relaxed);
        let rec_disk_rate = *self.app_state.rec_stats.disk_write_mbps.lock();
        let rec_segments = self.app_state.rec_stats.segments_written.load(std::sync::atomic::Ordering::Relaxed);
                                            ...
                                            VStack::new()
                                                ...
                                                .child(
                                                    HStack::new()
                                                        .items_center()
                                                        .gap_1_5()
                                                        .child(div().w_1_5().h_1_5().rounded_full().bg(gpui::rgb(0xff3b30)))
                                                        .child(
                                                            div()
                                                                .text_xs()
