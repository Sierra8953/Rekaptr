        ...
        div()
            ...
            .child(
                div()
                    ...
                    .child(self.render_clip_range(clip_start_prog, clip_end_prog, theme.tokens.primary))
                    // 3. Progress Fill
                    .child(self.render_progress_fill(progress, theme.tokens.primary))
                    // 4. Playhead
                    .child(self.render_playhead(progress, theme.tokens.foreground))
                    // 5. Clip Markers
                    .child(self.render_clip_marker(clip_start_prog, true, theme.tokens.primary, theme.tokens.background))
                    .child(self.render_clip_marker(clip_end_prog, false, theme.tokens.primary, theme.tokens.background))
                    // Interaction Layer
                    .child(
                        canvas(
