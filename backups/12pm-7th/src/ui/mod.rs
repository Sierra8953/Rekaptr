...
impl LumaWorkspace {
    ...
    pub fn toggle_recording(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if is_recording {
            ...
            let ts_offset_ns = if let Some((_, blocks)) = crate::utils::generate_session_playlist(&source_name) {
                            ...
                            loop {
                                ...
                                let _ = view_handle.update(&mut cx, |this, _| {
                                    ...
                                    if is_still_recording {
                                        if let (Some(start), Some(id)) = (this.recording_start_time, this.recording_session_id) {
                                            let duration = start.elapsed().as_secs_f64();
                                            if let Ok(db) = crate::db::GameDatabase::open(&game_dir_for_loop) {
                                                let _ = db.update_duration(id, duration);
                                            }
                                        }
                                    }
                                });
