...
pub fn generate_session_playlist(game_title: &str) -> Option<(PathBuf, Vec<crate::state::SessionBlock>)> {
    ...
    // 1. Try to get sessions from the database first
    ...
    // 2. Fallback or Sync: If DB is empty, or to find unregistered segments, scan disk
    if session_data.is_empty() {
        ...
        // Sync scanned data to DB if possible
        if let Ok(db) = crate::db::GameDatabase::open(&game_dir) {
            for (session_id, segments) in session_groups {
                let _ = db.register_session(session_id);
                // Calculate duration: (segments - 1) * 2.0
                let duration = (segments.len().saturating_sub(1)) as f64 * 2.0;
                let _ = db.update_duration(session_id, duration);
                session_data.push((session_id, duration));
            }
        }
    }
    ...
    // 3. Build session blocks for the timeline from the DB data
