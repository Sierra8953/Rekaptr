...
pub fn generate_session_playlist(game_title: &str, _active_session_id: Option<u64>) -> Option<(PathBuf, Vec<crate::state::SessionBlock>)> {
    ...
    // 1. Scan disk for all segments ordered by modification date
    ...
    // 2. Calculate total duration: assume 6.0s for all but the last segment for speed
    let mut total_duration = 0.0;
    let seg_count = segments.len();
    for (i, (_, path)) in segments.iter().enumerate() {
        if i == seg_count - 1 {
            total_duration += get_exact_duration(path);
        } else {
            total_duration += 6.0;
        }
    }
...
pub fn generate_master_playlist(game_title: &str) -> Option<PathBuf> {
    ...
    for (i, (_, seg)) in segments.iter().enumerate() {
        ...
        // Assume 6.0s for speed, but precisely measure the final segment
        let mut duration = 6.000;
        if i == seg_count - 1 {
                            ...
                            for entry in entries.filter_map(|e| e.ok()) {
                                if entry.path().is_dir() {
                                    ...
                                    if let Ok(seg_entries) = std::fs::read_dir(entry.path()) {
                                        ...
                                        // 3. ENFORCE PER-GAME RETENTION (6.0s duration)
                                        let mut total_game_duration = game_segments.len() as f64 * 6.0;
                                        for (path, _, _) in game_segments.iter() {
                                            if total_game_duration <= retention_secs { break; }
                                            let _ = std::fs::remove_file(path);
                                            total_game_duration -= 6.0;
                                        }
