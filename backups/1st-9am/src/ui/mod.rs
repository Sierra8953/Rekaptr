...
impl LumaWorkspace {
        ...
        cx.spawn(move |this: WeakEntity<Self>, cx: &mut AsyncApp| {
            ...
            async move {
                let playlist_path = if is_direct_file {
                    eprintln!("[UI] Loading direct file: {}", source_name_str);
                    Some(PathBuf::from(&source_name_str))
                } else {
                    let name_clone = source_name_str.clone();
                    eprintln!("[UI] Generating playlist for: {}", name_clone);
                    cx.background_executor().spawn(async move {
                        crate::utils::generate_session_playlist(&name_clone)
                    }).await
                };
                ...
                let _ = this.update(&mut cx, |this, cx| {
                    ...
                    if let Some(path) = playlist_path {
                        eprintln!("[UI] Successfully found playlist: {:?}", path);
                        ...
                        match Video::new_with_options(
                            ...
                            Ok(video) => {
                                eprintln!("[UI] Video source loaded successfully");
                                this.video_source = Some(video);
                            }
                            Err(e) => {
                                eprintln!("[UI] Failed to create Video instance: {:?}", e);
                                this.video_source = None;
                            }
                        }
                    } else {
                        eprintln!("[UI] ERROR: No segments or file found for: {}", source_name_str);
                        this.video_source = None;
                    }
                    cx.notify();
                });
    ...
    pub fn toggle_recording(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if is_recording {
            ...
            let timestamp = std::time::SystemTime::now()
                ...
                .unwrap()
                .as_secs();
            let session_dir_name = format!("session_{}", timestamp);
            ...
            match gst::parse::launch(&pipeline_str) {
                Ok(pipeline) => {
                    ...
                    // Phase 4: Manual D3D11 Device Sharing via GstContext
                    if let Some(handle) = self.app_state.d3d11_device.lock().as_ref() {
                        ...
                        eprintln!("[Phase 4] Shared D3D11 device handle {} with recording pipeline.", device_ptr);
                    }
    ...
    pub fn open_volume_popover(&mut self, track_idx: usize, cx: &mut Context<Self>) {
        ...
        let current_playback_volume = self.video_source.as_ref().map_or(100.0, |v| {
            v.read().mpv.get_property::<f64>("volume").unwrap_or(100.0)
        });
            ...
            root = root.child(
                div()
                    ...
                    .on_mouse_up(MouseButton::Left, cx.listener(move |this, _, _, cx| {
                        ...
                        if this.audio_track_volume_popover.is_some() && !this.is_adjusting_volume {
                            // If we click the backdrop while NOT dragging, close
                            // But since on_mouse_up is shared, let's use on_mouse_down for closing
                        }
                        cx.notify();
                    }))
                    ...
                    .child(
                        div()
                            .absolute()
                            .left(self.timeline_bounds.left() + px(95.0)) // Closer to headers
                            .top(px(self.popover_fixed_top - 70.0)) // Locked vertical position
                            .w(px(60.0))
                            .h(px(140.0))
                            .p_3()
