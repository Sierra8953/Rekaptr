...
impl Video {
    pub fn new_with_options(
        ...
        unsafe {
            ...
            // Phase 4: Explicitly share D3D11 device with MPV decoder
            if let Some(ptr) = d3d11_device_ptr {
                let ptr_str = format!("{}", ptr as usize);
                let _ = mpv.set_property("d3d11-device", &*ptr_str);
                eprintln!("[Phase 4] Shared D3D11 device with MPV decoder: {}", ptr_str);
            }
            ...
            mpv.set_property("demuxer-readahead-secs", "4").ok();
            mpv.set_property("demuxer-max-bytes", "32M").ok();
            mpv.set_property("demuxer-max-back-bytes", "16M").ok();
            ...
            let is_edl = path.ends_with(".edl");
            if is_edl {
                // For EDL files, we must ensure MPV doesn't try to guess the format incorrectly
                // and explicitly treat it as a playlist/lavf source.
                mpv.set_property("demuxer-lavf-format", "edl").ok();
            }
...
impl Video {
    ...
    pub fn set_display_size(&self, w: Option<u32>, h: Option<u32>) { let mut inner = self.0.write(); inner.display_width_override = w; inner.display_height_override = h; }
        pub fn refresh_segments(&self) {
            let inner = self.read();
            eprintln!("[Video] Refreshing segments for: {}", inner.source_name);
            if let Some(playlist_path) = crate::utils::generate_session_playlist(&inner.source_name) {
                let path_str = playlist_path.to_string_lossy().to_string();
                let _ = inner.mpv.command("loadfile", &[&path_str, "replace"]);
            }
        }
