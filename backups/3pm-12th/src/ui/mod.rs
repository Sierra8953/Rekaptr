            ...
            async move {
                loop {
                    ...
                    let _ = this.update(&mut cx, |this, _| {
                        ...
                        // Auto-hide preview controls after 3 seconds
                        if this.clip_to_preview.is_some() && this.show_preview_controls {
                            if this.last_preview_mouse_move.elapsed().as_secs() >= 3 {
                                this.show_preview_controls = false;
                                should_notify = true;
                            }
                        }
                    });
        ...
        cx.spawn(|this: WeakEntity<Self>, cx: &mut AsyncApp| {
            ...
            async move {
                ...
                let _ = this.update(&mut cx, |this, cx| {
                    ...
                    // Pre-calculate library rows for virtualized grid
                    ...
                    for chunk in game_titles.chunks(4) {
                        let titles_with_data: Vec<(String, usize, Option<String>)> = chunk.iter()
                            .map(|title| {
                                ...
                                // Just get the cached path if it exists, don't trigger fetch here to keep refresh_clips fast
                                let cached_path = this.app_state.artwork_cache.get(title).map(|v| v.value().clone()).flatten();
                    ...
                    this.library_items = rows;
                    this.clips_list_state.reset(this.library_items.len());
                                    ...
                                    match msg_view {
                                        gst::MessageView::Element(element_msg) => {
                                            if let Some(structure) = element_msg.structure() {
                                                ...
                                                if name == "splitmuxsink-fragment-opened" {
                                                    ...
                                                    // println!("[GStreamer] Fragment Opened: {}ns", running_time);
                                                    ...
                                                    if let Some(start_time) = current_fragment_start_time {
                                                        let duration_ms = (running_time.saturating_sub(start_time)) / 1_000_000;
                                                        
                                                        let old_path = std::path::PathBuf::from(&location);
                                                        let mut new_path = old_path.clone();
                                                        let file_stem = old_path.file_stem().unwrap().to_string_lossy().into_owned();
                                                        ...
                                                        if !file_stem.contains("ms") {
                                                            ...
                                                            cx.spawn(|cx: &mut AsyncApp| {
                                                                ...
                                                                async move {
                                                                    for _ in 0..25 {
                                                                        // Wait for GStreamer to release the handle (starting at 200ms)
                                                                        cx.background_executor().timer(std::time::Duration::from_millis(200)).await;
                                                                        if std::fs::rename(&old_path, &new_path).is_ok() {
                                                                            // println!("[GStreamer] Renamed finalized segment to {}ms", duration_ms);
                                                                            return;
                                                                        }
                                                                    }
                                                                    eprintln!("[GStreamer] Final rename failed after 5 seconds for {}", file_stem);
                                                                }
                                                            }).detach();
                                                        }
                                                    }
                                        ...
                                        gst::MessageView::Error(err) => {
                                            ...
                                            let _ = view_handle.update(&mut cx, |this, cx| {
                                                if let Some(any_window) = cx.windows().first() {
                                                    let _ = any_window.update(cx, |parent, window, cx| {
