                ...
                loop {
                    ...
                    let _ = this.update(&mut cx, |this, _| {
                        if let Some(v) = &this.video_source {
                            ...
                            // NEW: The Auto-Player! When a session ends, load the next playlist
                            if v.eof_reached() && !this.is_scrubbing {
                                let blocks = this.app_state.current_session_blocks.lock().clone();
                                let next_idx = this.playing_block_index + 1;
                                if next_idx < blocks.len() {
                                    this.playing_block_index = next_idx;
                                    let _ = v.load_file(&blocks[next_idx].playlist_path.to_string_lossy());
                                    v.set_paused(false);
                                }
                            }
