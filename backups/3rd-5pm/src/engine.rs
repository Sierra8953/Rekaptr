...
pub fn generate_pipeline_string(
    ...
    let location_str = format!("{}/segment_%05d.m4s", session_path.trim_end_matches(&['\\', '/'][..]));

    pipeline.push_str(&format!(
        " splitmuxsink name=sink muxer-factory=isofmp4mux location=\"{}\" max-size-time=2000000000 max-files={} send-keyframe-requests=true",
        location_str, max_files
    ));
