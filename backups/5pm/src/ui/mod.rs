...
impl LumaWorkspace {
    ...
    pub fn open_debug_window(&mut self, _window: &mut Window, cx: &mut Context<Self>) {
        if let Some(video) = self.video_source.clone() {
            let app_state = self.app_state.clone();
            let bounds = Bounds::centered(None, size(px(800.0), px(450.0)), cx);
            cx.open_window(WindowOptions {
                window_bounds: Some(WindowBounds::Windowed(bounds)),
                titlebar: Some(TitlebarOptions {
                    title: Some("Luma Debug Player".into()),
                    ..Default::default()
                }),
                ..Default::default()
            }, move |_window, cx| {
                cx.new(|_cx| StandalonePlayer { 
                    video,
...
impl Render for StandalonePlayer {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            .size_full()
            .relative()
            .child(crate::video_player::video(self.video.clone()).id("debug-video"))
            .child(
                div()
                    .absolute()
                    .bottom_4()
                    ...
                    .justify_center()
                    .child(
                        Button::new("debug-play-pause", if is_paused { "PLAY" } else { "PAUSE" })
                            .on_click(cx.listener(|this, _, _, cx| {
                                this.video.set_paused(!this.video.paused());
                                cx.notify();
                            }))
                    )
            )
