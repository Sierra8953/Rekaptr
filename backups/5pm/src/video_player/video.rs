...
impl Video {
    pub fn new_with_options(
        ...
        unsafe {
            ...
            SetPixelFormat(h_dc, pixel_format, &pfd).ok();
            ...
            d3d_device.CreateTexture2D(&desc, None, Some(&mut d3d_texture)).map_err(|e| Error::Lock)?;
            ...
            // Set vo=libmpv to prevent MPV from creating its own window
            ...
            // Create a 1x1 persistent image for the GPUI atlas key
            ...
            let worker = std::thread::spawn(move || {
                ...
                while alive_ref.load(Ordering::Acquire) {
                    let event = mpv_wait_event(ctx, 0.05);
                    if event.is_null() || (*event).event_id == mpv_event_id_MPV_EVENT_NONE { continue; }
            ...
            Ok(Video(Arc::new(RwLock::new(Internal {
                ...
                interop_texture_handle, d3d_texture, gl_texture, gl_fbo, render_image, alive,
    ...
    pub fn render_to_texture(&self) {
        ...
        unsafe {
            if wglMakeCurrent(inner.h_dc, inner.gl_context).is_err() {
                ...
                if !LOGGED_ERROR {
                    eprintln!("[Video] wglMakeCurrent failed in render_to_texture");
                    LOGGED_ERROR = true;
                }
                return;
            }
            ...
            if !(inner.interop.lock_objects)(inner.interop_device, 1, &mut handle) {
                static mut LOGGED_LOCK_ERROR: bool = false;
                if !LOGGED_LOCK_ERROR {
                    eprintln!("[Video] wglDXLockObjectsNV failed");
                    LOGGED_LOCK_ERROR = true;
                }
                return;
            }
            ...
            let err = mpv_render_context_render(inner.render_context, params.as_mut_ptr());
            if err < 0 {
                static mut LOGGED_RENDER_ERROR: bool = false;
                if !LOGGED_RENDER_ERROR {
                    eprintln!("[Video] mpv_render_context_render failed: {}", err);
...
impl Drop for Internal {
    fn drop(&mut self) {
        unsafe {
            wglMakeCurrent(self.h_dc, self.gl_context).ok();
...
impl Video {
    ...
    pub fn take_frame_ready(&self) -> bool { self.render_to_texture(); true }
    ...
    pub fn refresh_segments(&self) {}
    pub fn seek(&self, pos: Duration, _: bool) -> Result<(), Error> { self.read().mpv.command("seek", &[&format!("{}", pos.as_secs_f64()), "absolute"]).map_err(|_| Error::Bus) }
