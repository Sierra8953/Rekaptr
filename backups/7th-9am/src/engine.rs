...
pub fn generate_pipeline_string(
    ...
    let mut pipeline = format!(
        "{} ! videorate ! video/x-raw(memory:D3D11Memory),framerate={}/1 ! queue leaky=downstream max-size-buffers=5 ! {} ! queue max-size-buffers=10 ! {} ! {} ! {} ! queue ! sink.video",
    ...
    for (i, track) in audio_tracks.iter().enumerate() {
        ...
        pipeline.push_str(&format!(
            " {} ! queue ! audioconvert ! audioresample ! audio/x-raw,channels=2,rate=48000 ! volume volume={} name=vol_{} ! queue ! audioconvert ! avenc_aac ! aacparse ! queue ! sink.audio_{}",
            src, track.volume, i, i
        ));
    }

    pipeline.push_str(&format!(
        " splitmuxsink name=sink muxer-factory=isofmp4mux muxer-properties=\"properties,streamable=true,fragment-duration=2000\" location=\"{}/segment_%05d.m4s\" max-size-time=2000000000 max-files={} send-keyframe-requests=true",
        session_path.trim_end_matches(&['\\', '/'][..]),
        max_files
    ));
