...
impl LumaWorkspace {
    ...
    pub fn load_video(&mut self, source_name: &str, _window: &mut Window, cx: &mut Context<Self>) {
        ...
        if already_loaded && !is_direct_file {
            ...
            let current_pos = self.video_source.as_ref().map(|v| v.position().as_secs_f64()).unwrap_or(0.0);
            ...
            cx.spawn(move |this: WeakEntity<Self>, cx: &mut AsyncApp| {
                ...
                async move {
                    if let Some((_, blocks)) = cx.background_executor().spawn(async move {
                        crate::utils::generate_session_playlist(&name_for_bg)
                    }).await {
                        let _ = this.update(&mut cx, |this, cx| {
                            *this.app_state.current_session_blocks.lock() = blocks;
                            
                            // Re-load the master playlist to pick up new segments, 
                            // but seek back to the captured position immediately.
                            if let Some(v) = &this.video_source {
                                ...
                                if current_pos > 0.01 {
                                    let _ = v.seek(std::time::Duration::from_secs_f64(current_pos), false);
                                }
                                v.set_paused(false);
                            }
                            cx.notify();
    ...
    pub fn toggle_recording(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if is_recording {
            ...
            // Normalize path for GStreamer (forward slashes are more reliable)
            let s_dir_str = s_dir.to_string_lossy().replace('\\', "/");
            ...
            match gst::parse::launch(&pipeline_str) {
                Ok(pipeline) => {
                    ...
                    *self.app_state.pipeline.lock() = Some(pipeline);
                    ...
                    // NEW: Kick off a playlist generation after 2 seconds to ensure init.mp4 is extracted
                    let source_clone = source_name.clone();
                    cx.spawn(|_, cx: &mut AsyncApp| {
                        let mut cx = cx.clone();
                        async move {
                            cx.background_executor().timer(std::time::Duration::from_secs(3)).await;
                            cx.background_executor().spawn(async move {
                                crate::utils::generate_session_playlist(&source_clone);
                            }).await;
                        }
                    }).detach();
