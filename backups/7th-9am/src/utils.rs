...
pub fn generate_session_playlist(game_title: &str) -> Option<(PathBuf, Vec<crate::state::SessionBlock>)> {
    ...
    // 1. Find all session folders and sort chronologically
    let mut session_dirs = Vec::new();
    if let Ok(entries) = std::fs::read_dir(&game_dir) {
        for entry in entries.filter_map(|e| e.ok()) {
            let path = entry.path();
            if path.is_dir() && path.file_name().unwrap().to_string_lossy().starts_with("session_") {
    ...
    // 2. Build an isolated playlist for EACH session
    for session_dir in session_dirs {
        ...
        if let Ok(entries) = std::fs::read_dir(&session_dir) {
            for entry in entries.filter_map(|e| e.ok()) {
                ...
                if path.extension().map_or(false, |ext| ext == "m4s") {
                    ...
                    if name.starts_with("segment_") { segments.push(path); }
                }
            }
        }
        ...
        for (i, seg) in segments.iter().enumerate() {
            ...
            // Write the exact duration to the playlist
    ...
    // Generate the master playlist for unified playback
    generate_master_playlist(game_title);

    // Return the very first playlist to kick off the video player
    let first_playlist = session_blocks[0].playlist_path.clone();
    Some((first_playlist, session_blocks))
}
...
pub fn generate_master_playlist(game_title: &str) -> Option<PathBuf> {
    ...
    if session_dirs.is_empty() { return None; }

    let master_playlist_path = game_dir.join("master.m3u8");
    let mut m3u8_content = String::from("#EXTM3U\n#EXT-X-VERSION:4\n#EXT-X-TARGETDURATION:2\n#EXT-X-MEDIA-SEQUENCE:0\n#EXT-X-PLAYLIST-TYPE:VOD\n");

        for (session_idx, session_dir) in session_dirs.iter().enumerate() {
            let dir_name = session_dir.file_name().unwrap().to_string_lossy();
            let mut segments = Vec::new();
            if let Ok(entries) = std::fs::read_dir(&session_dir) {
                for entry in entries.filter_map(|e| e.ok()) {
            ...
            // Add discontinuity between sessions
            if session_idx > 0 {
                m3u8_content.push_str("#EXT-X-DISCONTINUITY\n");
            }
            
            // Add the initialization segment for the session
            m3u8_content.push_str(&format!("#EXT-X-MAP:URI=\"{}/init.mp4\"\n", dir_name));
            ...
            for (i, seg) in segments.iter().enumerate() {
                ...
                // Exclude segment_00000.m4s as it's renamed to init.mp4
                if file_name == "segment_00000.m4s" {
                    let init_path = session_dir.join("init.mp4");
                    if !init_path.exists() {
                        let _ = std::process::Command::new(get_ffmpeg_path())
                            .arg("-i").arg(&seg)
                            .arg("-f").arg("mp4")
                            .arg("-movflags").arg("frag_keyframe+empty_moov+separate_moof")
                            .arg("-codec").arg("copy")
                            .arg("-map").arg("0")
                            .arg("-t").arg("0")
                            .arg(&init_path)
                ...
                // Write the exact duration to the playlist
                m3u8_content.push_str(&format!("#EXTINF:{:.3},\n", duration));
                // Use relative path from master.m3u8 (which is in game_dir)
                                ...
                                if entry.path().is_dir() {
                                    ...
                                    if let Ok(seg_entries) = std::fs::read_dir(entry.path()) {
                                        for seg in seg_entries.filter_map(|e| e.ok()) {
                                            if seg.path().extension().map_or(false, |ex| ex == "m4s" || ex == "mp4" || ex == "ts") {
