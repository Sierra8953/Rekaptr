...
impl LumaWorkspace {
        ...
        for chunk in game_titles.chunks(4) {
            ...
            let row = HStack::new()
                ...
                .children(chunk.iter().map(|title| {
                    ...
                    Self::render_game_card_vertical(title.clone(), count, view_handle.clone())
                }));
    ...
    fn render_game_card_vertical(title: String, clip_count: usize, view_handle: WeakEntity<Self>) -> impl IntoElement {
        let theme = use_theme();
