...
pub fn get_storage_root() -> PathBuf {
    // Check if E: exists, otherwise fallback to C:
    let e_drive = PathBuf::from("E:\\LumaRecordings");
    if e_drive.exists() {
        e_drive
    } else {
...
pub fn fetch_storage_usage() -> (u64, u64) {
    let root = get_storage_root();
    let clips_dir = root.join("Clips");
    
    // Ensure directories exist so they can be counted
    let _ = std::fs::create_dir_all(&root);
    let _ = std::fs::create_dir_all(&clips_dir);

    let clips_bytes = get_dir_size(&clips_dir);
    let mut sessions_bytes = 0;
    ...
    if let Ok(entries) = std::fs::read_dir(&root) {
        for entry in entries.filter_map(|e| e.ok()) {
            ...
            if path.is_dir() {
                ...
                // Count everything in the root EXCEPT Clips, Cache, and hidden folders as session data
                if name != "Clips" && name != "Cache" && !name.starts_with(".") {
                    sessions_bytes += get_dir_size(&path);
                }
            }
        }
    }
    
    (clips_bytes, sessions_bytes)
}
                        ...
                        if let Ok(entries) = std::fs::read_dir(&root_dir) {
                            for entry in entries.filter_map(|e| e.ok()) {
                                if entry.path().is_dir() {
                                    ...
                                    let retention_mins = if game_title.to_lowercase() == "monitor" {
                                        config.global_video.retention_minutes as f64
                                    } else {
                                        config.game_registry.iter().find(|(t, _)| clean_title(t).to_lowercase() == game_title.to_lowercase())
                                    ...
                                    if let Ok(seg_entries) = std::fs::read_dir(entry.path()) {
                                        ...
                                        // Add a 10-second grace period to prevent oscillation
                                        let target_retention = retention_secs + 10.0;

                                        let mut deleted_count = 0;
                                        for (path, _, _) in game_segments.iter() {
                                            if total_game_duration <= target_retention { break; }
                                        ...
                                        if deleted_count > 0 {
                                            log::info!("[Cleanup] Pruned {} segments for '{}'. Current duration: {:.1}s / target: {:.1}s", 
                                                deleted_count, game_title, total_game_duration, retention_secs);
                                        }
