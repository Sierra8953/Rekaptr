...
impl Video {
    pub fn new_with_options(
        ...
        unsafe {
            ...
            mpv.set_property("hwdec", "auto-safe").ok(); // Direct D3D11 hardware decoding (Zero-Copy)
            ...
            mpv.set_property("interpolation", "no").ok(); // Disable interpolation to save CPU/GPU
            mpv.set_property("prefetch-playlist", "yes").ok();
            mpv.set_property("cache", "yes").ok();
            mpv.set_property("demuxer-thread", "yes").ok();
            mpv.set_property("demuxer-readahead-secs", "10").ok(); // Increase readahead
            mpv.set_property("demuxer-max-bytes", "1000000000").ok(); // 1GB cache
            mpv.set_property("demuxer-max-back-bytes", "200000000").ok();
            mpv.set_property("vd-lavc-threads", "1").ok(); // Reduce threads if hwdec is active
            mpv.set_property("hr-seek-framedrop", "yes").ok(); // Save CPU during seeks
            mpv.set_property("video-sync", "audio").ok(); // Standard sync strategy
