                                            ...
                                            div()
                                                ...
                                                .on_mouse_move(cx.listener(|this, event: &MouseMoveEvent, _window, cx| {
                                                    if this.is_adjusting_volume {
                                                        // Fallback global mouse capture release handler
                                                    }
                                                }))
                                                .child(
                                                    canvas(
                                                        ...
                                                        },
                                                        {
                                                            ...
                                                            move |bounds, _, window, cx| {
                                                                // Global drag handler attached to the canvas bounds to capture mouse
                                                                // safely even if it leaves the physical 8px width.
                                                                let height = bounds.size.height;
                                                                let fill_height = height * (current_vol_percentage / 150.0).clamp(0.0, 1.0);
                                                                
                                                                let fill_rect = Bounds::new(
                                                                    point(bounds.left(), bounds.top() + height - fill_height),
                                                                    size(bounds.size.width, fill_height)
                                                                );
                                                                window.paint_quad(fill(fill_rect, theme.tokens.primary).corner_radii(px(4.0)));
                                                                
                                                                // Thumb indicator
                                                                let thumb_rect = Bounds::new(
                                                                    point(bounds.left() - px(4.0), bounds.top() + height - fill_height - px(4.0)),
                                                                    size(px(16.0), px(8.0))
                                                                );
                                                ...
                                                // Invisible interaction layer that extends wider than the visual track
                                                .child(
                                                    div()
                                                        .absolute()
                                                        .inset_0()
                                                        .mx_neg_6() // Make the drag area wide enough to catch clumsy mouse movements
                                                        ...
                                                        .child(
                                                            canvas(
                                                                move |_, window, cx| {
                                                                    let layout_id = window.request_layout(Style { size: size(relative(1.0).into(), relative(1.0).into()), ..Default::default() }, [], cx);
                                                                    (layout_id, ())
                                                                },
                                                                {
                                                                    let view = view.clone();
                                                                            ...
                                                                            if this.is_adjusting_volume {
                                                                                ...
                                                                                if track_height > px(0.0) {
                                                                                    ...
                                                                                    if let Some(track_idx) = this.last_audio_track_volume_popover {
                                                                                        ...
                                                                                        // Only trigger update if it actually changed to prevent flutter
                                                                                        if (this.playback_volumes[track_idx] - volume).abs() > 0.5 {
