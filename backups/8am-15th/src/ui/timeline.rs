...
impl LumaWorkspace {
    pub fn render_timeline(&self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // Global Mouse Handlers for Volume Slider
        if self.is_adjusting_volume {
            let view_handle = view.clone();
            window.on_mouse_event(move |event: &MouseMoveEvent, phase, _window, cx| {
                if phase == DispatchPhase::Bubble {
                    let _ = view_handle.update(cx, |this, cx| {
                        let bounds = this.volume_slider_bounds;
                        let track_height = bounds.size.height;
                        if track_height > px(0.0) {
                            let relative_y = (event.position.y - bounds.top()).clamp(px(0.0), track_height);
                            let percentage = 1.0 - (relative_y / track_height).clamp(0.0, 1.0);
                            let volume = (percentage * 150.0).clamp(0.0, 150.0) as f64;
                            ...
                            if let Some(idx) = this.last_audio_track_volume_popover {
                                ...
                                if (this.playback_volumes[idx] - volume).abs() > 0.1 {
        ...
        HStack::new()
            ...
            .when_some(self.audio_track_volume_popover, |this, track_idx| {
                ...
                let current_vol_percentage = self.playback_volumes.get(track_idx).copied().unwrap_or(100.0) as f32;
                            ...
                            VStack::new()
                                ...
                                .child(
                                    div()
                                        ...
                                        .items_center()
                                        .justify_center()
                                        .child(div().text_xs().font_weight(FontWeight::BOLD).child(format!("{:.0}%", current_vol_percentage)))
                                )
                                .child(
                                    div()
                                        .flex_1()
                                                    ...
                                                    canvas(
                                                        move |_, window, cx| {
                                                            let layout_id = window.request_layout(Style {
                                                                size: size(px(20.0).into(), relative(1.0).into()),
                                                                ..Default::default()
                                                            }, [], cx);
                                                            (layout_id, ())
                                                        },
                                                        move |bounds, _, window, cx| {
                                                            let _ = view_handle.update(cx, |this, cx| {
                                                                this.volume_slider_bounds = bounds;
                                                                
                                                                let track_height = bounds.size.height;
                                                                let track_width = px(8.0);
                                                                let track_rect = Bounds::new(
                                                                    point(bounds.center().x - px(4.0), bounds.top()),
                                                                    size(track_width, track_height)
                                                                );

                                                                // Draw background track
                                                                window.paint_quad(fill(track_rect, theme.tokens.muted).corner_radii(px(4.0)));

                                                                // Draw fill
                                                                let fill_height = track_height * (this.volume_slider_last_value / 150.0).clamp(0.0, 1.0);
                                                                let fill_rect = Bounds::new(
                                                                    point(track_rect.left(), track_rect.top() + track_height - fill_height),
                                                                    size(track_width, fill_height)
                                                                );
                                                                window.paint_quad(fill(fill_rect, theme.tokens.primary).corner_radii(px(4.0)));

                                                                // Draw thumb
                                                                let thumb_rect = Bounds::new(
                                                                    point(track_rect.left() - px(4.0), track_rect.top() + track_height - fill_height - px(4.0)),
                                                                    size(px(16.0), px(8.0))
                                                                );
                                                                window.paint_quad(fill(thumb_rect, gpui::white()).corner_radii(px(4.0)));

                                                                // Handle dragging logic here to use accurate bounds from this paint pass
                                                                if this.is_adjusting_volume {
                                                                    let mouse_pos = window.mouse_position();
                                                                    let relative_y = (mouse_pos.y - bounds.top()).clamp(px(0.0), track_height);
                                                                    let percentage = 1.0 - (relative_y / track_height).clamp(0.0, 1.0);
                                                                    let volume = (percentage * 150.0).clamp(0.0, 150.0) as f64;
                        ...
                        canvas(
                            ...
                            },
                            {
                                ...
                                move |bounds, _, window, cx| {
                                    ...
                                    // Optimization: Only loop through marks that are actually visible on screen.
                                    // Calculation based on x = left - scroll + (width * zoom) * (i / num_marks)
                                    // Since num_marks = 20 * zoom, this simplifies to x = left - scroll + width * (i / 20)
                                    let start_i = ((scroll * 20.0) / width.0).floor() as i32;
                                    let end_i = (((scroll + width.0) * 20.0) / width.0).ceil() as i32;

                                    for i in start_i.max(0)..=end_i.min(num_marks) {
