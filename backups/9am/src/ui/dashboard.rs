                    ...
                    .on_click(cx.listener({
                        ...
                        move |this: &mut Self, _, _, cx| {
                            ...
                            let cached_path = {
                                ...
                                state.playlist_cache.get(&title).map(|v| v.clone())
                            };

                            if let Some(path) = cached_path {
                                this.load_video(&path, cx);
                            } else if let Some(playlist_path) = crate::utils::generate_session_playlist(&title) {
                                {
                                    let state = this.app_state.lock();
                                    state.playlist_cache.insert(title.clone(), playlist_path.clone());
                                }
                                this.load_video(&playlist_path, cx);
                            }
                            cx.notify();
