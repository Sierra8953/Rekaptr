...
impl LumaWorkspace {
    pub fn new(app_state: Arc<Mutex<AppState>>, cx: &mut Context<Self>) -> Self {
        ...
        if let Some(playlist_path) = crate::utils::generate_session_playlist("monitor") {
            workspace.load_video(&playlist_path, cx);
        } else {
            workspace.load_video(std::path::Path::new("C:\\LumaRecordings\\monitor\\view.m3u8"), cx);
        }
    ...
    pub fn toggle_play_pause(&mut self, cx: &mut Context<Self>) {
        if let Some(v) = &self.video_source {
            ...
            if was_paused {
                ...
                // Regenerate the playlist to include any segments added while paused
                if let Some(playlist_path) = crate::utils::generate_session_playlist(source) {
                    self.app_state.lock().playlist_cache.insert(source.to_string(), playlist_path);
                }
            }
            v.set_paused(!was_paused);
            cx.notify();
        }
    }
    ...
    pub fn save_clip(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        self.show_toast("Clip Saved", Some("Your recording has been saved to C:\\LumaRecordings"), adabraka_ui::overlays::toast::ToastVariant::Success, window, cx);
    }
            ...
            match gst::parse::launch(&pipeline_str) {
                Ok(element) => {
                    ...
                    if pipeline.set_state(gst::State::Playing).is_ok() {
                        ...
                        // Regenerate playlist immediately so we can see the "Live" buffer
                        if let Some(playlist_path) = crate::utils::generate_session_playlist(&source_name) {
                            state.playlist_cache.insert(source_name.clone(), playlist_path.clone());
                            drop(state);
                            self.load_video(&playlist_path, cx);
                        } else {
                            drop(state);
                        }
