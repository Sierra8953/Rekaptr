...
pub(crate) struct Internal {
    ...
    pub(crate) info: Arc<RwLock<gst_video::VideoInfo>>,
...
impl Video {
    ...
    pub fn new_with_options(uri: &url::Url, options: VideoOptions) -> Result<Self, Error> {
        ...
        let pipeline_str = format!(
            "playbin uri=\"{}\" video-sink=\"videoconvert ! appsink name=gpui_video caps=video/x-raw,format=NV12\"",
            uri.as_str()
        );
        ...
        let video_sink = if let Ok(bin) = video_sink.clone().downcast::<gst::Bin>() {
            bin.by_name("gpui_video").ok_or_else(|| Error::AppSink("gpui_video".into()))?
        } else {
            video_sink
        };
    ...
    pub fn from_gst_pipeline_with_options(
        ...
        let caps = cleanup!(video_sink.static_pad("sink").and_then(|p| p.current_caps()).ok_or(Error::Caps))?;
        let info = cleanup!(gst_video::VideoInfo::from_caps(&caps).map_err(|_| Error::Caps))?;

        let info_state = Arc::new(RwLock::new(info));
        let info_ref = Arc::clone(&info_state);
                ...
                while let Some(msg) = bus_ref.timed_pop(gst::ClockTime::from_seconds(0)) {
                    match msg.view() {
                        ...
                        MessageView::Error(err) => {
                            eprintln!("[Video] GStreamer Error: {} (from {:?})", err.error(), err.src().map(|s| s.name()));
                            if let Some(dbg) = err.debug() {
                                eprintln!("[Video] Debug info: {}", dbg);
                            }
                        }
                        _ => {}
                    }
                ...
                if let Err(_) = (|| -> Result<(), gst::FlowError> {
                    let maybe_sample =
                        if pipeline_ref.current_state() != gst::State::Playing {
                            video_sink.try_pull_preroll(gst::ClockTime::from_mseconds(32))
                        } else {
                            video_sink.try_pull_sample(gst::ClockTime::from_mseconds(32))
                        };
                    ...
                    *last_frame_time_ref.lock() = Instant::now();
                    ...
                    // Update VideoInfo if caps have changed
                    if let Some(caps) = sample.caps() {
                        if let Ok(new_info) = gst_video::VideoInfo::from_caps(caps) {
                            let mut info_guard = info_ref.write();
                            if info_guard.width() != new_info.width() || info_guard.height() != new_info.height() {
                                *info_guard = new_info;
                            }
                        }
                    }
    ...
    pub fn size(&self) -> (i32, i32) {
        let inner = self.read();
        let info_guard = inner.info.read();
        (info_guard.width() as i32, info_guard.height() as i32)
    }
    ...
    pub fn display_size(&self) -> (u32, u32) {
        let inner = self.read();
        let info_guard = inner.info.read();
        let natural_w = info_guard.width();
        let natural_h = info_guard.height();
