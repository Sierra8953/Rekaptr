                ...
                loop {
                    ...
                    let _ = this.update(&mut cx, |this, _| {
                        if let Some(v) = &this.video_source {
                            ...
                            // Determine current session block based on absolute position
                            let pos = v.position().as_secs_f64();
                            let blocks = this.app_state.current_session_blocks.lock();
                            for (idx, block) in blocks.iter().enumerate() {
                                if pos >= block.timeline_offset_secs && pos < (block.timeline_offset_secs + block.duration_secs) {
                                    if this.playing_block_index != idx {
                                        this.playing_block_index = idx;
                                        should_notify = true;
                                    }
                                    break;
                                }
    ...
    pub fn load_video(&mut self, source_name: &str, _window: &mut Window, cx: &mut Context<Self>) {
        ...
        if already_loaded && !is_direct_file {
            ...
            cx.spawn(move |this: WeakEntity<Self>, cx: &mut AsyncApp| {
                let mut cx = cx.clone();
                async move {
                    if let Some((path, blocks)) = cx.background_executor().spawn(async move {
                        crate::utils::generate_session_playlist(&source_name_str)
                    }).await {
                        let _ = this.update(&mut cx, |this, cx| {
                            *this.app_state.current_session_blocks.lock() = blocks;
                            if let Some(v) = &this.video_source {
                                let _ = v.load_file(&path.to_string_lossy());
                                v.set_paused(false);
                            }
                            cx.notify();
                        });
        ...
        cx.spawn(move |this: WeakEntity<Self>, cx: &mut AsyncApp| {
            let mut cx = cx.clone();
            async move {
                let (playlist_path, blocks) = if is_direct_file {
                    (Some(PathBuf::from(&source_name_str)), Vec::new())
                } else {
                    let name_clone = source_name_str.clone();
                    if let Some((path, b)) = cx.background_executor().spawn(async move {
                ...
                let _ = this.update(&mut cx, |this, cx| {
                    ...
                    *this.app_state.current_session_blocks.lock() = blocks.clone();
                    ...
                    if let Some(sink) = pipeline.by_name("sink") {
                        ...
                        sink.connect("format-location", false, move |args| {
                            std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                                ...
                                // Just use the standard segment_%05d naming directly.
                                // The cleanup thread we updated in utils.rs will handle
                                // the max-files deletion safely while protecting the 00000 header.
                                let fname = format!("{}/segment_{:05}.m4s", s_path, id);
