...
impl Video {
    pub fn new_with_options(
        ...
        unsafe {
            ...
            mpv.set_property("hwdec", "d3d11va-copy").ok(); // D3D11 is safest for Windows
            ...
            mpv.set_property("gapless-audio", "yes").ok();
            mpv.set_property("prefetch-playlist", "yes").ok();
...
impl Video {
    ...
    pub fn set_display_size(&self, w: Option<u32>, h: Option<u32>) { let mut inner = self.0.write(); inner.display_width_override = w; inner.display_height_override = h; }
        ...
        pub fn append_file(&self, path: &str) -> Result<(), Error> {
            let inner = self.read();
            inner.mpv.command("loadfile", &[path, "append-play"]).map_err(|_| Error::Bus)
        }
