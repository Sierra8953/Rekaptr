...
impl WindowsWindow {
    pub(crate) fn new(
        ...
        let (mut dwexstyle, mut dwstyle) = if params.kind == WindowKind::PopUp {
            ...
            let mut dwstyle = WS_SYSMENU | WS_CLIPCHILDREN;
...
impl PlatformWindow for WindowsWindow {
    ...
    fn direct3d11_device(&self) -> Option<*mut std::ffi::c_void> {
        use windows::core::Interface;
        let state = self.0.state.borrow();
        Some(state.renderer.devices.device.as_raw())
    }

    fn raw_window_handle(&self) -> Option<*mut std::ffi::c_void> {
        Some(self.0.hwnd.0 as *mut std::ffi::c_void)
    }
