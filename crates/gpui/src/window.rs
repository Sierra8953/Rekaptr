...
impl Window {
    ...
    /// Returns the Direct3D 11 device handle for this window, if available.
    pub fn direct3d11_device(&self) -> Option<*mut std::ffi::c_void> {
        self.platform_window.direct3d11_device()
    }

    /// Returns the raw window handle for this window, if available.
    pub fn raw_window_handle(&self) -> Option<*mut std::ffi::c_void> {
    ...
    /// Paint a hardware texture into the window.
    pub fn paint_hardware_texture(
        &mut self,
        bounds: Bounds<Pixels>,
        corner_radii: Corners<Pixels>,
        data: Arc<RenderImage>,
        ...
        // Ensure the tile exists in the atlas with the correct size
        let tile = self
            .sprite_atlas
            .get_or_insert_with(&params.into(), &mut || {
                Ok(Some((
                    size,
                    Cow::Owned(vec![0u8; (size.width.0 * size.height.0 * 4) as usize]),
                )))
            })?
            .expect("Callback above only returns Some");

        // Perform GPU-to-GPU copy
        self.sprite_atlas.update_tile_from_hardware(&tile, texture_ptr)?;
