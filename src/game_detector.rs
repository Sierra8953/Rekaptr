...
unsafe extern "system" fn enumerate_windows_callback(hwnd: HWND, lparam: LPARAM) -> windows::core::BOOL {
    ...
    if IsWindowVisible(hwnd).as_bool() {
        // PERF: GetWindowTextW is faster than GetWindowThreadProcessId + sysinfo lookup.
        // Filter out nameless windows first before doing expensive OS process lookups.
        ...
        if len > 0 {
            ...
            // PERF: Consolidated PID lookup and process name extraction.
            let mut pid = 0u32;
            ...
            // Fallback if process lookup fails
            state.windows.push(WindowInfo {
                title,
                hwnd: hwnd.0 as u64,
