...
impl LumaWorkspace {
    ...
    pub fn toggle_recording_ext(
        &mut self,
        explicit_hwnd: Option<u64>,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    fn stop_recording(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let game_dir_for_fixup = crate::utils::get_storage_root().join(crate::utils::clean_title(
            &self
                .selected_source
                .clone()
                .unwrap_or_else(|| "monitor".to_string()),
    ...
    fn start_recording(
        ...
        *self.app_state.recording.phase.lock() = crate::state::RecordingPhase::Starting;
        ...
        let game_dir =
            crate::utils::get_storage_root().join(crate::utils::clean_title(&source_name));
        ...
        *self.app_state.recording.current_recording_duration.lock() =
            ts_offset_ns as f64 / 1_000_000_000.0;
                ...
                let pipeline = match pipeline.dynamic_cast::<gst::Pipeline>() {
                    ...
                    Err(_) => {
                        *self.app_state.recording.phase.lock() = crate::state::RecordingPhase::Idle;
                        self.show_toast(
                            "Pipeline Error",
                            Some("Failed to initialize recording pipeline."),
                            adabraka_ui::overlays::toast::ToastVariant::Error,
                            window,
                            cx,
                        );
                    ...
                    if let Ok(appsrc) = src.dynamic_cast::<gstreamer_app::AppSrc>() {
                        if !track.app_targets.is_empty() {
                            for app_name in &track.app_targets {
                                ...
                                std::thread::spawn(move || {
                                    log::info!(
                                        "[Recording] Starting capture for target '{}' on track {}",
                                        app_name_clone,
                                        i
                                    );
                                    if let Err(e) = crate::engine::start_app_capture(
                                        app_name_clone,
                                        None,
                                        appsrc_clone,
                                    ) {
                            ...
                            let app_name =
                                if track.device_name.is_empty() || track.device_name == "Default" {
                            ...
                            std::thread::spawn(move || {
                                log::info!(
                                    "[Recording] Starting fallback capture for '{}' on track {}",
                                    app_name,
                                    i
                                );
                                if let Err(e) =
                                    crate::engine::start_app_capture(app_name, target_pid, appsrc)
                                {
                        ...
                        gst::MessageView::Element(element_msg) => {
                            if let Some(structure) = element_msg.structure() {
                                ...
                                if name == "splitmuxsink-fragment-opened" {
                                    let running_time: u64 =
                                        structure.get("running-time").unwrap_or(0);
                            ...
                            let _ = view_handle.update(&mut cx, |this, cx| {
                                if let Some(any_window) = cx.windows().first() {
                                    let _ = any_window.update(cx, |_, window, cx| {
                                        this.show_toast(
                                            "Recording Error",
                                            Some("The GStreamer pipeline crashed."),
                                            adabraka_ui::overlays::toast::ToastVariant::Error,
                                            window,
                                            cx,
                                        );
    ...
    async fn handle_fragment_closed(
        ...
        let Some(start_time) = fragment_start_time else {
            return;
        };
...

        {
            ...
            if duration_secs > 0.0 {
                ...
                *stats.disk_write_mbps.lock() =
                    file_size as f64 / duration_secs / (1024.0 * 1024.0);
            }
        }
