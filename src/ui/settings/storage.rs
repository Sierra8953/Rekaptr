...
use adabraka_ui::charts::pie_chart::{
    PieChart, PieChartLabelPosition, PieChartSegment, PieChartSize,
};
use adabraka_ui::prelude::*;
use gpui::*;

impl RekaptrWorkspace {
    pub(crate) fn render_settings_storage(
        &self,
        theme: &Theme,
                                    ...
                                    VStack::new()
                                        ...
                                        .child(
                                            div()
                                                ...
                                                .child(format!("{:.1} GB", total_gb)),
                                        ),
                                )
                                .child(
                                    div().flex_1().child(
                                        PieChart::new(vec![
                                            PieChartSegment::new("Clips", clips_gb)
                                                .color(theme.tokens.primary),
                                            PieChartSegment::new("Sessions", sessions_gb)
                ...
                Card::new().content(
                    VStack::new()
                        ...
                        .child(settings_row(
                            ...
                            settings_toggle(
                                "toggle-autodel",
                                self.settings_form_auto_delete_enabled,
                                vh.clone(),
                                |this, cx| {
                                    this.settings_form_auto_delete_enabled =
                                        !this.settings_form_auto_delete_enabled;
                                    let mut config = crate::config::AppConfig::load();
                                    config.auto_delete_clips_days =
                                        if this.settings_form_auto_delete_enabled {
                                            ...
                                            .on_click({
                                                ...
                                                move |_, _, cx| {
                                                    let _ = vh.update(cx, |this, cx| {
                                                        this.settings_form_auto_delete_days = (this
                                                            .settings_form_auto_delete_days
                                                            - 1)
                                                        .max(1);
                                                        let mut config =
                                                            crate::config::AppConfig::load();
                                                        config.auto_delete_clips_days = Some(
                                                            this.settings_form_auto_delete_days,
                                                        );
                                                        config.save();
                                                        cx.notify();
                                                    });
