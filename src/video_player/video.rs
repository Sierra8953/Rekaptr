...
type GlFramebufferTexture2D = unsafe extern "system" fn(
    target: u32,
    attachment: u32,
    textarget: u32,
    texture: u32,
...
type WglDXLockObjectsNV =
    unsafe extern "system" fn(device: HANDLE, count: i32, objects: *mut HANDLE) -> bool;
    ...
    pub fn new_with_options(
        ...
        unsafe {
            if OPENGL32_DLL.0.is_null() {
                OPENGL32_DLL =
                    windows::Win32::System::LibraryLoader::LoadLibraryW(w!("opengl32.dll"))
                        .map_err(|e| {
                            Error::OpenGL(format!("LoadLibrary opengl32.dll failed: {:?}", e))
                        })?;
            }

            // The D3D11 device in AppState is AddRef'd and outlives all Video instances.
            let d3d_device: ID3D11Device =
                std::mem::transmute_copy(&d3d11_device_ptr.ok_or(Error::Lock)?);
            ...
            let instance = windows::Win32::System::LibraryLoader::GetModuleHandleW(None)
                .map_err(|e| Error::Mpv(format!("GetModuleHandleW failed: {:?}", e)))?;
            ...
            if pixel_format == 0 {
                return Err(Error::OpenGL("ChoosePixelFormat failed".into()));
            }
            let _ = SetPixelFormat(h_dc, pixel_format, &pfd);

            let gl_context = wglCreateContext(h_dc)
                .map_err(|e| Error::OpenGL(format!("wglCreateContext failed: {:?}", e)))?;
            wglMakeCurrent(h_dc, gl_context).ok();

            let load_ext = |name: &str| -> Result<*const c_void, Error> {
                let cname = CString::new(name)
                    .map_err(|_| Error::Interop(format!("Invalid extension name: {}", name)))?;
            ...
            if interop_device.is_invalid() {
                return Err(Error::Interop("wglDXOpenDeviceNV failed".into()));
            }
            ...
            let d3d_texture =
                d3d_texture.ok_or_else(|| Error::OpenGL("CreateTexture2D returned None".into()))?;
            ...
            if interop_texture_handle.is_invalid() {
                return Err(Error::Interop("wglDXRegisterObjectNV failed".into()));
            }

            let mut gl_fbo = 0;
            (gl_procs.gen_framebuffers)(1, &mut gl_fbo);
            (gl_procs.bind_framebuffer)(GL_FRAMEBUFFER, gl_fbo);
            (gl_procs.framebuffer_texture2d)(
                GL_FRAMEBUFFER,
            ...
            let worker = std::thread::spawn(move || {
                ...
                while alive_ref.load(Ordering::Acquire) {
                    ...
                    if (*event).event_id == mpv_event_id_MPV_EVENT_SHUTDOWN {
                        break;
                    }
                }
            });
            ...
            Ok(Video(Arc::new(RwLock::new(Internal {
                ...
                alive,
                worker: Some(worker),
                source_name: options.source_name.unwrap_or_default(),
                width,
                height,
...
impl Video {
    ...
    pub fn size(&self) -> (i32, i32) {
        let inner = self.read();
        (inner.width as i32, inner.height as i32)
    }
    ...
    pub fn set_paused(&self, p: bool) {
        let _ = self.read().mpv.set_property("pause", p);
    }
    ...
    pub fn set_display_size(&self, w: Option<u32>, h: Option<u32>) {
        let mut inner = self.0.write();
        inner.display_width_override = w;
        inner.display_height_override = h;
    }
    ...
    pub fn seek(&self, pos: Duration, _: bool) -> Result<(), Error> {
        self.read()
            .mpv
            .command("seek", &[&format!("{}", pos.as_secs_f64()), "absolute"])
            .map_err(|_| Error::Bus)
    }
